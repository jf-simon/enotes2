                ...
                const char* src EINA_UNUSED)
{
  ...
  Evas_Object* ly_help = eina_list_nth(list_values, 5);

  Evas_Object* edje_obj = elm_layout_edje_get(ly_help);
  ...
  // LAYOUT CREATE //
  ly_help = elm_layout_add(win);
  snprintf(buf, sizeof(buf), "%s/themes/enotes.edj", elm_app_data_dir_get());
  elm_layout_file_set(ly_help, buf, "help");
  evas_object_size_hint_weight_set(ly_help, EVAS_HINT_EXPAND, EVAS_HINT_EXPAND);

  elm_win_resize_object_add(win, ly_help);
  evas_object_show(ly_help);
...
//   ///
  ...
  elm_object_part_content_set(ly_help, "table", tb);

  evas_object_event_callback_add(ly_help, EVAS_CALLBACK_KEY_DOWN, key_down_help, win);
  evas_object_event_callback_add(ly_help, EVAS_CALLBACK_MOUSE_DOWN, _enotes_help_move, win);
  elm_layout_signal_callback_add(ly_help, "close_help", "close_help", enotes_win_help_close1, win);
