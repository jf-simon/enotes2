...
#define COPYRIGHT                                                         \
"Copyright © 2022 Simon Tischer <simon@t-tischer.de> for the enotes app"  \
...
_my_conf_descriptor_init(void)
{
   ...
   MY_CONF_SUB_ADD_BASIC(categories, EET_T_STRING);
...
enotes_win_setup(Note* list_data)      // create the note (Evas_Objects) and show it
{
   ...
   // categories check
   if(list_data->categories == NULL || !strcmp(list_data->categories, "") || !strcmp(list_data->categories, activ_cat))
   ...
   if (ci_border_enabled == EINA_TRUE)
   ...
   enotes_all_objects = eina_list_append(enotes_all_objects, list_data->categories);
